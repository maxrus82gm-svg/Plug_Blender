...
`АНАЛИЗ МОДИФИКАТОРОВ` работает локально и без AI: выдели active object в Object Mode, нажми **Получить модификаторы**, выбери один modifier и нажми **Сравнить параметры**. Add-on сравнивает его с freshly-created modifier того же type в текущем Blender и показывает только changed properties. RNA metadata defaults не используются. Служебные `__ASTRO_MODELER_TEMP_*` datablocks удаляются; возможный dirty `.blend` — известное ограничение V1. Контекст необязателен: пустое поле использует стандартную инструкцию. Для объяснения пользователь отдельно просит Codex разобрать изменения; Blender сам conversation не запускает. Пользовательские строки секции централизованы с русским языком V1; полноценный RU/EN selector пока не добавлен.
